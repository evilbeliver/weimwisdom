...
describe('shared shell components', () => {
  it('renders the header navigation', () => {
    ...
    expect(screen.getByRole('link', { name: 'WeimWisdom' })).toBeInTheDocument();
  ...
  it('renders the footer summary and links', () => {
    ...
    expect(screen.getByText(/Breed facts, training plans/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'Rescue' })).toBeInTheDocument();
  });
